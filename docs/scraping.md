...
- `run_fee_scrape()` clears `form_fees_address` before repopulating it,
...
- Splitting is now driven by multi-valued **fee** cells, not category
  lists alone. If a filing-category cell has an explanatory `<ol>`/`<ul>`
  but the fee cells contain a single fee (for example H-1B/L fraud-fee
  conditions or I-131 IMMVI descriptions), the parser keeps the row as
  one form row instead of creating duplicate Excel rows.
...
- `_row_to_fee_item`'s `filing_category` fallback no longer grabs an
  ...
  table is just two fee columns). It now stores a blank category instead
  of inventing `"General Filing"`, because that text is not present on
  USCIS's page.
- `GET /api/v1/fees` returns `paper_fee_text`, `online_fee_text`, and
  `fee_details` in addition to numeric `paper_fee`/`online_fee`. This is
  important for Excel comparisons because some USCIS values are not a
  single number, such as `$1,500 or $750`.
...
`run_fee_scrape()` now clears the table first, so a normal refresh no
longer needs a manual `TRUNCATE TABLE form_fees_address;` beforehand.
