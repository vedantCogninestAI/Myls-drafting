...
- **No `drafts` table.** The generated draft only exists inside the LangGraph checkpoint (MySQL-backed, survives restarts, but not queryable). There's no way to fetch a case's current/past drafts outside of the `/generate` or `/approve` response itself — no `GET` endpoint, no relational row. Deliberately deferred until after seeing real agent output; not yet revisited.
