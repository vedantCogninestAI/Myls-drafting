...
**This is the sole origination point for a `process_type`.** Cases (`docs/database.md`) carry no `process_type` of their own — a case is just a client's ingested data, reusable across any draft type. `process_type` only comes into existence here, the first time templates are uploaded for a new string; `GET /api/v1/template-generation/process-types` (below) then becomes the canonical list of every `process_type` that exists, which draft generation (`docs/draft.md`) reads from independently at generate time.
...
## Listing Available Process Types

`GET /api/v1/template-generation/process-types` — `TemplateRepository.get_distinct_process_types()`, a plain `SELECT DISTINCT process_type FROM templates`. This is the enum a caller uses to pick a `process_type` for draft generation (`docs/draft.md`) or to check whether a given string already exists before uploading templates under a possibly-new, possibly-duplicate name.

```json
{ "process_types": ["N-400 Cover Letter", "I-130", "I-751"] }
```

Registered before `GET /{process_type}` on the router so `process-types` resolves as the static route rather than being captured as a `process_type` value.
...
| File | Role |
|---|---|
| `app/api/v1/endpoints/template_generation.py` | `POST /{process_type}` (upload + cap check), `GET /{process_type}` (fetch), `GET /process-types` (list) — no graph, straight `endpoint → service`/`repository` |
...
| `app/repositories/template.py` | `TemplateRepository` — `count_by_process_type()`, `save_templates()`, `get_by_process_type()`, `get_distinct_process_types()` |
| `app/models/template.py` | `Template` model (`templates` table) |
| `app/schemas/template.py` | `TemplateUploadResponse` / `TemplateResult`, `TemplateListResponse` / `TemplateItem`, `ProcessTypeListResponse` |
